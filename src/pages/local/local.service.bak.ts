...
export class LocalService {
  ...
  getRecruitList(obj) {
    ...
    let params = this.toolsService.param(obj);
    ...
    return new Promise(resolve => {
      ...
      function getCityInfo(status, result) {
        if (status == 'complete') {
          ...
          let lastPosition=that.storageService.getItem('LAST_POSITION');
          if(lastPosition){
              this.parentId=lastPosition.areaId
          }
          else{
              this.parentId=11959;
          }
          that.loadDataByParentId.emit(this.parentId);
        }
      }
    })
  }
